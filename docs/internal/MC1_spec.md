...
**说明**：内部方法学文档；团队协作文档（如 revision tracker）不在公开仓库中。
...
**代码对齐项**：`scripts/dataset.py` 主分析 `LOS_ICU_MIN = 1`（天，即 ≥24h）；完整 t₀ 时间戳来自 `data/mimic_t0_labs.parquet`（SQL 导出）；Excel 仅有聚合特征时使用 **proxy 分层**（见 `scripts/build_t0_cohort.py`）。
...
| 评估子集 | 与 `preds.pkl` **同一 patient/stay ID** | 见 `result/analysis/manifest.json` |
